...
def get_user_cart_service(user_id):
    ...
    Eager load items và product của items.
    """
...
def add_to_cart_service(user_id, product_id, quantity):
    ...
    db.session.commit()
    return {"message": "Item added to cart", "cart_item": cart_item}, 201
...
def update_cart_item_service(user_id, cart_item_id, quantity):
    ...
    db.session.commit()
    return {"message": "Cart item updated", "cart_item": cart_item}, 200
